...
export default function SpeakerSlider() {
    ...
    const handleSlideChange = () => {
        if (swiperRef.current && swiperRef.current.swiper) {
            ...
            if (swiperInstance.activeIndex === SpeakersImagesData.length) {
                navigate('/slide/3');
    ...
    return (
        <Swiper
            ...
            ref={swiperRef}
        >
            {SpeakersImagesData.map((image, index) => (<SwiperSlide key={`slide-${index}`}>
                <LazyLoadImage
                    src={image}
                    alt={`Slide ${index + 1}`}
                    ...
                    afterLoad={() => {
                        ...
                        if (imageElement) {
                            ...
                            imageElement.style.transition = 'transform 1.2s ease-in-out, opacity 3s ease-in-out'; // Animasyonu başlat
                        }
                    }}
                />
            </SwiperSlide>
